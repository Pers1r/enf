    ...
    def post(self, request, slug):
        ...
        if existing_item:
            ...
            if total_quantity > product_size.stock:
                    return JsonResponse({
                    'error': f'Cannot add {product_size.stock} items. Only {product_size.stock} items available',
                }, status=400)
    ...
    def post(self, request, item_id):
        ...
        if quantity == 0:
            ...
            if quantity > cart_item.product_size.stock:
                return JsonResponse({
                    'error': f'Cannot add {quantity} items. Only {cart_item.product_size.stock} items available',
                }, status=400)
...
class ClearCartItemView(CartMixin ,View):
    def post(self, request, item_id):
        cart = self.get_cart(request)
        cart.clear()
